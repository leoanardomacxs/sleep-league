...
const getCycles = () => {
  ...
  return [4, 5, 6].map((count) => {
    ...
    return {
      ...
      quality: count === 6 ? "Ótimo" : count === 5 ? "Bom" : "Ok",
      icon: count === 6 ? "🌟" : count === 5 ? "✨" : "💤",
    };
  });
};
            ...
            <div className="px-6 pt-3 pb-10">
              ...
              <div className="bg-surface rounded-2xl p-3.5 mb-5 flex items-start gap-3">
                ...
                <p className="text-xs text-muted-foreground font-body leading-relaxed">
                  Considerando ~15 min para adormecer. Acordar no final de um ciclo completo reduz a sensação de cansaço.
                </p>
              </div>
                ...
                {cycles.map((cycle, i) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="text-2xl">{cycle.icon}</div>
                      <div className="text-left">
                        <p className="text-2xl font-display tabular-nums text-foreground tracking-tight">
                          {cycle.time}
                        </p>
                        <p className="text-xs text-muted-foreground font-body mt-0.5">
                          {cycle.hours} de sono · {cycle.count} ciclos
                        </p>
                      </div>
                    </div>
                    <span
                      className={`text-xs font-ui font-bold px-3 py-1.5 rounded-full ${
                        cycle.count === 6
              ...
              <div className="mt-4 flex items-center justify-center gap-2">
                ...
                <p className="text-xs text-muted-foreground font-body">
                  Recomendado: <span className="text-accent font-bold">5–6 ciclos</span> para melhor recuperação
                </p>
              </div>
            </div>
          </motion.div>
